class NoteIO {
      ...
      this.listView.innerHTML = `
            <li id="${placeholderId}">
                <div>
                    ...
                    <textarea id="content-${placeholderId}" readonly>${randomPlaceholder.content}</textarea>
                </div>
                <button type="button" id="copy-${placeholderId}" class="button-style">
                  <i class="fi fi-br-clone"></i>
                </button>
            </li>
        `;
  ...
  clearButtonReset() {
    let noteList = JSON.parse(localStorage.getItem("notes")) || {};
    if (Object.entries(noteList).length > 0) this.clearAllButton.style.display = "flex";
  }

  loadCards() {
    this.clearButtonReset()
    let noteList = JSON.parse(localStorage.getItem("notes")) || {};
    this.listView.innerHTML = "";
  ...
  createShareDialog(id) {
    ...
    if (!document.getElementById("shareContentDialog")) {
      ...
      dialogShare.innerHTML += `
        <div id="shareContent">
          <nav>
            <h2>Share note</h2>
            <button class="button-style outline" onclick="document.getElementById('shareContentDialog').remove()">
                <i class="fi fi-br-cross-small"></i>
              </button>
          </nav>
          <div class="share-content">
            <p id="text-outline" class="button-style outline">${noteList[id].content}</p>
            
            <div class="button-group">
              <button type="button" class="button-style outline" onclick="window.open('https://wa.me/?text=${encodeURIComponent(
        (noteList[id].title ? `*${noteList[id].title}*\n\n` : '') + noteList[id].content
      )}', '_blank')">
                <i class="fi fi-brands-whatsapp"></i>
              </button>
          
              <button type="button" class="button-style outline" onclick="window.location.href = 'mailto:?subject=${encodeURIComponent(
        noteList[id].title ? `<strong>${noteList[id].title}</strong>\n\n` : ''
